...
class ListService {
    async handle() {
        // Receita do mês
        const appointmentsMonth = await database_1.default.appointment.findMany({
            where: {
                dateTime: {
                    gte: startOfMonth,
                    lt: endOfMonth,
        ...
        // Receita de Hoje
        const appointmentsToday = await database_1.default.appointment.findMany({
            where: {
                dateTime: {
                    gte: startOfToday,
                    lt: endOfToday,
        ...
        const mostRequestedServices = await database_1.default.appointment.groupBy({
            by: ['serviceId'],
            _count: {
                serviceId: true,
            },
        ...
        // Receita anual
        const appointmentsYear = await database_1.default.appointment.findMany({
            where: {
                dateTime: {
                    gte: startOfYear,
                    lt: endOfYear,
        ...
        const upcomingAppointments = await database_1.default.appointment.findMany({
            where: {
                dateTime: {
                    gte: today,
                },
